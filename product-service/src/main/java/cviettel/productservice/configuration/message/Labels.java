...
import cviettel.productservice.constants.Constants;
import cviettel.productservice.util.StringPool;
...
//            log.error("Can not get label for key {} , return default value.", key, ex);
    ...
    public static String getLanguageFromRequest() {
        ...
        String value = request.getHeader(Constants.DEFAULT_LOCALE);


        if (value != null) {
            value = value.trim();
            value = Labels.replace(value, StringPool.RETURN_NEW_LINE, StringPool.NEW_LINE,0);
